//! Gemini LLMPort 어댑터 — module-llm `GeminiRawClient` thin wrapper.
...
use module_llm::{GeminiRawClient, LlmRawPort};
use tracing::info;
...
impl GeminiAdapter {
    pub fn new(api_key: String, model: String) -> Self {
        Self {
            raw: GeminiRawClient::new(api_key, model.clone()),
            model,
        }
    }
}
...
impl LLMPort for GeminiAdapter {
    async fn classify_and_process(
        ...
        info!("Gemini 호출 ({}): {} ({} chars)", self.model, filename, content.len());
...
impl file_pipeline_core::ports::outbound::OutboundManifest for GeminiAdapter {
    fn id(&self) -> &str { "fp-outbound-llm-gemini" }
    ...
    fn capabilities(&self) -> file_pipeline_core::ports::output::ResourceCapabilities {
        file_pipeline_core::ports::output::ResourceCapabilities::standard("gemini")
    }
}
