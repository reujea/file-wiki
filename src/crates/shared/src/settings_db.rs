...
impl SettingsDb {
    ...
    pub fn open_or_migrate(data_dir: &Path) -> Result<Self> {
        ...
        if pipeline_toml.exists() && !db.has_data_in("config")? {
            ...
            let config = crate::config::PipelineConfig::load(&pipeline_toml)?;
        ...
        if doc_types_toml.exists() && !db.has_data_in("doc_types")? {
            ...
            let registry = crate::config::load_doc_type_registry(&doc_types_toml)?;
        ...
        if !db.has_data_in("config")? {
            ...
            let default_config = crate::config::PipelineConfig::default_config();
    ...
    /// 테이블에 데이터가 있는지 확인
    fn has_data_in(&self, table: &str) -> Result<bool> {
    ...
    /// 크레덴셜 전체 조회
    pub fn list_credentials(&self) -> Result<Vec<crate::config::LlmCredential>> {
        let conn = self.conn.lock().expect("mutex poisoned");
    ...
    /// 크레덴셜 저장 (upsert)
    pub fn save_credential(&self, cred: &crate::config::LlmCredential) -> Result<()> {
        let conn = self.conn.lock().expect("mutex poisoned");
        conn.execute(
    ...
    pub fn migrate_from_config(&self, config: &crate::config::PipelineConfig) -> Result<()> {
    ...
    /// DB에�� PipelineConfig 복원
    pub fn to_pipeline_config(&self) -> Result<crate::config::PipelineConfig> {
        let all = self.get_all_config()?;
        // 전체를 하나�� JSON object로 재구성
        ...
        let config: crate::config::PipelineConfig = serde_json::from_value(serde_json::Value::Object(root))
            .unwrap_or_else(|e| {
                tracing::warn!("DB→PipelineConfig 복원 실패, 기본값 사용: {}", e);
                crate::config::PipelineConfig::default_config()
            });
        Ok(config)
    }
    ...
    pub fn save_snapshot(&self, snap: &crate::config_snapshot::ConfigSnapshot) -> Result<()> {
    ...
    pub fn list_snapshots(&self, limit: usize) -> Result<Vec<crate::config_snapshot::ConfigSnapshot>> {
        let conn = self.conn.lock().expect("settings.db lock");
        let mut stmt = conn.prepare(
        ...
        let rows = stmt.query_map([limit as i64], |row| {
            ...
            Ok(crate::config_snapshot::ConfigSnapshot {
                id: row.get(0)?,
    ...
    pub fn get_snapshot(&self, id: &str) -> Result<Option<crate::config_snapshot::ConfigSnapshot>> {
        let conn = self.conn.lock().expect("settings.db lock");
        let mut stmt = conn.prepare(
            "SELECT id, created_at, config_hash, config_backup, profile_json, applied_paths, metrics_json, rolled_back, rollback_reason
        ...
        if let Some(row) = rows.next()? {
            let applied_paths: String = row.get(5)?;
            Ok(Some(crate::config_snapshot::ConfigSnapshot {
                id: row.get(0)?,
                created_at: row.get(1)?,
                config_hash: row.get(2)?,
                config_backup: row.get(3)?,
...
mod tests {
    ...
    fn test_credential_crud() {
        let db = setup();
        let cred = crate::config::LlmCredential {
            id: "c1".into(),
    ...
    fn test_migrate_from_config() {
        let db = setup();
        let config = crate::config::PipelineConfig::default_config();
...
"#;
    ...
    fn test_roundtrip_config() {
        let db = setup();
        let original = crate::config::PipelineConfig::default_config();
    ...
    fn test_get_section_as_typed() {
        ...
        let comp: crate::config::CompressionConfig = db.get_section_as("compression").expect("typed");
    ...
    fn test_open_or_migrate_from_toml() {
        ...
        let db = SettingsDb::open_or_migrate(dir.path()).expect("migrate");
        assert!(db.has_data_in("config").expect("check"));
    ...
    fn test_open_or_migrate_idempotent() {
        ...
        // 1차: 기본값으로 DB 생성
        {
            let db = SettingsDb::open_or_migrate(dir.path()).expect("first");
            assert!(db.has_data_in("config").expect("check"));
        }

        // 2차: 이미 DB 존재 → 마이그레이션 스킵
        {
            let db = SettingsDb::open_or_migrate(dir.path()).expect("second");
            assert!(db.has_data_in("config").expect("check"));
        }
    }
