---
updated: 2026-06-05 (Phase 200/201/202 placeholder 진입 — plugin 도메인 신규 §추가. host = 파일 가공만. 외부 기능 모두 plugin. 단일 진실원: prd/research/plugin-architecture-2026-06-04.md)
---
...
## Plugin 도메인 (Phase 200~202 placeholder 진입, 2026-06-05)

> **단일 진실원** (메타 룰 19, lesson 75 자기 적용): plugin 인터페이스 + 매니페스트 + 권한 매핑은 본 §가 단일 진실원. `architecture.md`는 결정 맥락(Why)과 상태 전이(How many)만 보유. 신규 plugin 추가 시 본 §를 먼저 갱신, architecture.md는 결정 맥락만 추가.

### Phase 200~202 placeholder 진입 상태
...
| **202** | IpcMessage + IpcResponse + HostEvent wire 타입 | `_rust_module/fp-plugin-protocol/src/lib.rs` (wire 타입만, 실 전송 미진입) |
...
| 타입 / trait | 위치 | 비고 |
|-------------|------|------|
| `Plugin` trait | `_rust_module/fp-plugin-sdk/src/lib.rs` | placeholder — `fn id(&self) -> &str` 만. Phase 202 본진입 시 handle_request / on_event 추가 |
...
| `registry.rs` | discover + enable/disable + (Phase 202 대기) call | `PluginRegistry::{new, discover, enable, disable, call, count, list, get}` |
| `mod.rs` | 재노출 + Phase docstring | 외부에서 `core::plugin::*` 로 접근 |
...
### PluginError 7 variants (registry.rs)

`Io / ManifestParse / ApiVersionMismatch / UnknownPermission / DuplicatePluginId / PluginNotFound / IpcNotYetImplemented` — `thiserror` 파생.
...
└── plugins/                       ← 첫 실행 자동 생성 (config.rs::create_all)
    └── {plugin_id}/               ← 예: io.file-pipeline.search/
        ...
        └── fp-plugin-search.exe   ← Phase 202 IPC 실행 binary (다음 진입)
```
