...
## 메타 룰 18: lesson 본문의 추정 사항은 다음 phase에서 재검증 의무 (2026-06-17 정식 승격)

(lesson 46 G-1/G-4 + lesson 50 + lesson 78 누적 4건 → 2026-06-17 정식 승격, 메타 룰 23 §승격 3요소 모두 충족 + 본 cycle 메타 룰 25 자기 적용 정합)

(메타 룰 12 확장) lesson 본문의 **"~로 추정", "~인 듯", "원인 불명" 표현**은 다음 phase 종결 시 재검증 의무. 단순히 lesson에 기록만 하면 stale 위험.

### 사례 (4회 누적, 2026-06-17 본 세션 +1: lesson 78 step-o2/o3 host 추정 5회 사이클 vs 실측 2회)
...
| **lesson 78 step-o2/o3 (2026-06-17)** | **host 직전 cycle 진입 명세 추정 "lesson #14 R1 family 5회 사이클" (super-trait 광역 sniff + workspace cycle + reqwest multipart + 추가 미박힘 2종 = 5회 추정)** | **실측 2회 사이클만 발생 (workspace cycle 정정 + reqwest multipart feature 박힘 = 2회). 5회 추정은 step-o2 의 24 어댑터 + SensitivityAware + 10 mock 발견을 별도 사이클로 카운트한 host 측 과장. sub-pattern 3종 정형 = super-trait 광역 sniff / workspace cycle 회피 / feature flag 박힘 (lesson 78 본문 박힘)** |

추정 빗나간 비율 = **4/4 = 100%**. 추정은 신뢰할 수 없다는 패턴 확정 강화.
...
**lesson 78 강화 (2026-06-17 정식 승격)**:
- [ ] **handoff 메시지 내 "N회 사이클 예상" 추정 = bb 측 expected_cycles 필드 박힘 + step 종결 시 actual_cycles 비교 의무** (lesson 78 step-o2/o3 host 예상 5회 vs 실측 2회 = 60% 빗나감 패턴 정형)
- [ ] **lesson 본문의 sub-pattern 정형 의무** — 추정 빗나감의 본질 sub-pattern (예: super-trait 광역 sniff / workspace cycle 회피 / feature flag 박힘) 박힘
- [ ] **추정 빗나간 lesson 본문은 다음 cycle handoff cache 에 sub_patterns_learned_this_cycle 필드 박힘** (학습 영구화)

### 정식 승격 (2026-06-17 메타 룰 23 §3요소 모두 충족)

- [x] 누적 ≥ 3건 — **4건 도달** ✅ (lesson 46 G-1 + lesson 46 G-4 + lesson 50 + lesson 78)
- [x] 체크리스트: "추정 키워드 grep + 재검증 + ✅/❌ 명시 갱신 + sub-pattern 정형 + handoff cache 영구화" 5요소
- [x] META.md 본문 등재 ✅ (2026-06-17 정식 승격 시점)
...
### 누적 사례 (19건 도달, 2026-06-16 본 세션 +2건: telegram storage 분류 + outbound 우산 본질 재정의)
| lesson | phase | 영역 |
|--------|------|------|
...
| **76 file-pipeline 단독 git** | **본 세션 2026-06-10** | **Phase 202 본진입 시점 — file-pipeline 단독 git 첫 설정 + http://gitlab.bi.co.kr/reujea/file.git origin. 정책 경계 = file-pipeline·_rust_module·tasty 3 워크스페이스 독립 git 보존 합의 (메타 룰 22 17건째)** |
| **77 (a) telegram storage 분류** | **본 세션 2026-06-16 진입 중반** | **사용자 발화 "원격 저장소를 텔레그램 추가 구성 추가" → Notion 패턴 vs notification 통합 vs raw input source 3 옵션 → 사용자 옵션 A (Notion storage backend 차용). plugin-architecture §3-C-1 신규 (telegram = storage + notify 양쪽). 메타 룰 22 18건째** |
| **77 (b) outbound 우산 본질 재정의** | **본 세션 2026-06-16 직후** | **사용자 발화 "storageAdaptor 외부 연계 plugin 공통화 + 헥사고날 adaptor 패턴 통일" → 단일 plugin+adapter / 독립 plugin 공통 trait / outbound 공통 우산 3 옵션 → 사용자 옵션 C (outbound 공통 우산). plugin-architecture §3-C 본문 전면 재정의 (`fp-outbound-{category}-{name}` prefix + OutboundManifest super-trait + 25 어댑터 통일). telegram-storage-1 plan 즉시 폐기 → outbound-umbrella-1 6 step 신설. 메타 룰 22 19건째 — 단일 세션 사용자 발화로 본질 재정의 발생 + 진행 중인 plan 즉시 폐기 첫 사례** |
...
### 누적 사례 (13건 도달, 2026-06-16 본 세션 +5: lesson 76 + lesson 77 + architecture.md 갱신 + domain-map.md 갱신 + architecture.md 본문 stale 정정 자기 적용)
| lesson | 영역 | 자기 적용 |
|--------|------|---------|
...
| **76** | **bundle-cycle B1~B4 단일 세션 종결 직후 lesson 76 등재** | **47 단위/통합 테스트 PASS 직후 + Phase 202 본진입 직후 lesson 등재 + INDEX 갱신 + META 자기 적용. 메타 룰 25 9건째** |
| **77** | **본 cycle 3 plan 병행 11 step succeeded 직후 lesson 77 등재** | **outbound 우산 본질 재정의 + service.rs 1681→615줄 분해 + 24 어댑터 manifest impl 종결 직후 lesson 77 등재 + INDEX 갱신 + META 메타 룰 22 19건 + 25 10건 + 30 13건 누적 동시 갱신. 메타 룰 25 10건째 (자기 적용 영역 host 자율 진행 = "다음 tasty loop 진행" 간접 트리거 해석)** |
| **77 (a) architecture.md 자기 적용** | **lesson 77 등재 직후 2026-06-16** | **lesson 77 + META 갱신 직후 spec/architecture.md §누적 변경 요약 (2026-06-16) 신규 영역 박힘 (3 plan 진척 + service.rs -1066 + outbound 우산 본질 재정의 표 + 메타 룰 누적 + 회귀 검증 + 잔여 host 결정 7건) + 헤더 updated 갱신. 메타 룰 25 11건째 + 30 14건째** |
| **77 (b) domain-map.md 자기 적용** | **architecture.md 직후 2026-06-16** | **spec/domain-map.md Phase 207 어댑터 → plugin 매핑 표 outbound 우산 재정의 정합 (fp-plugin- → fp-outbound- prefix + storage 5→6 + telegram storage+notify 양쪽 + 어댑터 수 표 추가) + 디렉토리 경로 정정 4 영역 (verification → verify + reranking → rerank + notification → notify) + RemoteStoragePort 우산 박힘 명시. 메타 룰 25 12건째 + 30 15건째 + 메타 룰 1 sub-rule 1g 회피 (단일 진실원 plugin-architecture-2026-06-04.md §3-C 위임 표시)** |
| **77 (c) architecture.md 본문 stale 정정** | **domain-map.md 직후 2026-06-16** | **architecture.md L899 service.rs 1620줄 stale 후속 명시 (lesson 77 §누적 변경 요약 1681→615줄 -1066 -63% 위임) + 본문 디렉토리 drift 5 라인 정정 (notification → notify 3 영역 + 파일 트리 다이어그램 3 라인 verification/reranking → verify/rerank). lesson 77 등재 후 본문 stale 정합 정정 = 자기 적용 의무 7단계 완성. 메타 룰 25 13건째** |
...
### 누적 사례 (16건 도달, 2026-06-16 본 세션 +4: plugin-architecture §3-C 재정의 직후 lesson 77 등재 + architecture.md 자기 적용 + domain-map.md 자기 적용 + architecture.md 본문 stale 정정)
| lesson | phase | 패턴 |
|--------|------|------|
...
| **77 plugin-architecture §3-C 재정의 직후 lesson 77 등재** | **본 세션 종결 시점 2026-06-16** | **사용자 옵션 C (outbound 우산 본질 재정의) 직후 plugin-architecture §3-C 본문 재정의 + lesson 77 등재 + INDEX 갱신 + META.md 메타 룰 22/25/30 누적 갱신 동시. host 자율 영역 진입 (사용자 "현행화" 명시 부재, "다음 tasty loop 진행" 반복 발화를 간접 트리거로 해석). 메타 룰 30 자기 적용 13건째 + 메타 룰 25 10건째 (자기 적용 의무)** |
| **77 (a) architecture.md §누적 변경 요약 신규 영역 박힘** | **lesson 77 + META 직후 2026-06-16** | **lesson 77 등재 직후 spec/architecture.md §누적 변경 요약 (2026-06-16) 신규 영역 박힘 + 헤더 updated 라인 갱신 (2026-06-10 → 2026-06-16). plan 별 산출 표 + service.rs -1066줄 + outbound 우산 본질 재정의 표 + 메타 룰 누적 + 회귀 검증 + 잔여 host 결정 7건 동시. 메타 룰 30 자기 적용 14건째** |
| **77 (b) domain-map.md outbound 우산 정합** | **architecture.md 직후 2026-06-16** | **architecture.md 직후 spec/domain-map.md Phase 207 어댑터 → plugin 매핑 표 outbound 우산 재정의 정합 (fp-plugin- → fp-outbound- prefix + storage 5→6 + telegram storage+notify 양쪽 + 어댑터 수 표 추가) + 디렉토리 경로 정정 4 영역 (verification → verify + reranking → rerank + notification → notify) + RemoteStoragePort 우산 박힘 명시. 메타 룰 30 자기 적용 15건째 + 메타 룰 1 sub-rule 1g 회피 (단일 진실원 plugin-architecture-2026-06-04.md §3-C 위임 표시 정합)** |
| **77 (c) architecture.md 본문 stale 정정** | **domain-map.md 직후 2026-06-16** | **lesson 77 등재 + architecture.md §누적 변경 요약 신규 + domain-map.md 갱신 후 본문 stale 6 영역 자기 grep 정합 — L899 service.rs 1620줄 stale 후속 명시 (1681→615 -1066 위임) + L1504/L2027/L2156 디렉토리 drift 정정 3 영역 (notification → notify) + L2616~2618 파일 트리 다이어그램 3 라인 (verification/reranking → verify/rerank). 메타 룰 30 자기 적용 16건째 + 본 cycle 자기 적용 7단계 종결 (lesson 77 → META 1차 → architecture.md → domain-map.md → META 자기 적용 → architecture.md 본문 stale → META 자기 위반 회피)** |
