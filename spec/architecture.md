---
updated: 2026-06-18 (cycle 7: module-storage-db-1 plan 완결 d2~d6. **d5+d6**: adapters/driven/{settings/sqlite.rs 2292→11줄, vector_db/local_store.rs 1079→8줄}를 module-storage-db thin re-export shim으로 축소(총 3371→19줄, 코드 중복 해소, 호출처 변경 0). adapters/Cargo.toml +module-storage-db path. 검증: workspace check --all 경고0 / Tauri / nextest(bench제외) 435/435+26skip + module-storage-db 자체 32/32(이관 단위테스트 손실0, 합 467 정합). work_queue.rs=DB결합0 core잔류. plan d1~d6 종결. 이전 d2+d3+d4: **d3**: DB 본체(sqlite.rs 2292 + local_store.rs 1079)를 `_rust_module/module-storage-db` 단일 impl crate로 이관 — fp-domain-types만 의존(VectorDBPort/SettingsRepoPort 구현), file_pipeline_core 무의존, 순환 0. api crate는 폐기(포트=fp-domain-types 단일 진실원). 역방향 결합 제거: HostToolRepo::refresh가 adapters HostToolDetector 직접 호출하던 것 → trait를 저장전용(list_host_tools/host_tools_count/replace_host_tools)으로 재설계, 감지+저장 조합은 shared/host_tools_cache.rs 자유함수가 담당(감지=adapters/shared, 저장=DB impl). d4(workspace 등록)=d3 흡수. _rust_module members 28 + deps +7. 빌드 5종 PASS(module-storage-db/_rust_module/workspace/Tauri/nextest bench제외 467+26skip 회귀0). 미완 d5: adapters 원본 잔존(코드 2곳, d5 thin wrapper 시 re-export 대체). **d2**: fp-domain-types crate 신설. 순수 도메인 타입 + 출력 포트 trait를 `_rust_module/fp-domain-types`로 추출 = core/module-storage-db 공용 단일 진실원. core는 re-export shim 7 (models/config_models/settings_models/vec_io/hooks/output/settings_repo) + 분리 모듈 re-export 4 (verification_thresholds/kg_types/chunk_quality/crossref_optimizer) → 호출처 변경 0. DB 본체(sqlite.rs 2292 + local_store.rs 1079) core 의존 6 모듈이 전부 fp-domain-types로 닫힘 → d1 BLOCKED 해소 + d3 게이트 통과(module-storage-db → fp-domain-types 단방향, core 무관). 빌드 5종 PASS (fp-domain-types / core / workspace check --all 경고0 / _rust_module 26멤버 / Tauri). 사고 2건 lesson 81 (PowerShell UTF-8 한글 오독 + git HEAD 미커밋 손실 복구). 이전 cycle 5: mcp-removal-1 완결 — MCP 전체 폐기. mcp_server.rs 2139줄 삭제 + 37 도구 + CLI Serve 삭제 + Tauri get_mcp_tool_catalog_full 삭제 + webapp-design.md MCP 카드 폐기 + architecture.md current MCP 수치 0 정정. 코드 내 MCP 실체 0. 본질 재정의 3차 — host=파일 가공만, 외부 연계는 plugin(헥사고날 adaptor)으로 통일. 과거 Phase 로그는 히스토리 보존. 빌드 3종 PASS(workspace check 0 / Tauri check 0 / nextest 회귀 0). step-m5 완결. cycle 4: prep-unlock(SettingsDb 본체) done — settings-db-split-1 plan 완결. SettingsDb 86 메서드 → adapters/driven/settings/sqlite.rs (shared/settings_db.rs 2230→138줄 re-export + open_or_migrate 자유함수). 누적 19 step succeeded)
---
...
> ⚠️ **본 §은 2026-06-16 outbound-umbrella-1 작업의 역사 기록이다.** 여기서 도입한 `OutboundManifest` super-trait는 **2026-06-18 plugin-sdk-1 step-p7에서 완전 폐기**되었다 (본질 재정의 3차 "Output adapter = raw I/O 만" 정합 — 도메인 메타데이터 우산 → plugin manifest + raw_transport 4 채널 대체). 폐기 단일 진실원 = `spec/deprecated.md` §삭제됨. 아래 표/요약은 도입 시점 사실로 읽을 것.
...
config 자동 결정  →   AutoResolution/Sensitive    →   VectorDBPort (LocalVectorStore)
...
crates/adapters/ driven/(llm/claude+anthropic+openai+ollama+gemini+fallback+chunked_agent+prompts,
                 ...
                 driving/(watcher+retry_queue, auto_resolution, auto_sensitive)    (2026-06-18 terminal_* → auto_* config 자동 결정)
