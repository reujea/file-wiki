...
| host 잔류 | watcher + WorkQueue + Preprocess (PDF/Excel/한글) + Chunk + Metadata 구조 + DB 영속 + audit 코어 + Plugin Registry + MCP server + Tauri 진입점 |
...
**단일 진실원**: `prd/research/plugin-architecture-2026-06-04.md` (2026-06-04). §3 host 잔류 경계 / §4 plugin 28 MCP 매핑 / §5 SDK + Protocol 정의 / §6 Phase 200~209 단계 / §7 위험 매트릭스 / §9 기존 흡수 영역 재배치.
...
- **6 도메인 → 1 host + 11 MCP plugin + 24 어댑터 plugin** — 28 MCP 도구 / 23 어댑터 → 11 + 24 = 35 plugin 멤버
- **외부 흡수 정책 진화** — lesson 30 "인프라 선구현" 자연 진화: plugin 단위 발행으로 host 영향 0

### 사이드 발견 (본 결정으로 자연 해소)

- MCP 카탈로그 단일 진실원 분산 우려 → `PluginRegistry::mcp_router`가 매니페스트 수집 → 동적 카탈로그 (Phase 92 H3 일치성 테스트 패턴 자연 흡수)
...
| `get_mcp_tool_catalog_full` | 26 MCP 도구 다차원 분류 (mutates/category/cost) | Phase 92 H3 |
...
- **🧰 MCP 도구 분류**: 26 도구 카테고리별 그룹화 + mutates/cost 컬럼. `refresh-mcp-catalog` 액션
...
- `process_file_legacy` 삭제로 service.rs **2034 → 약 1620줄** (-414줄, -20.4%)
...
- 발견 위치: `modals/cli/tests/notification_integration.rs` / `modals/cli/tests/real_env_tests.rs` / `crates/adapters/src/driven/notification/format.rs`
...
- Phase 72: 26 어댑터 단위 테스트 +25 (notification/format, stub, storage)
...
로컬 파일을 자동 분류·가공·압축·색인하여 Claude Code(MCP)로 검색 가능하게 만드는 Windows 데스크톱 파이프라인. LocalVectorStore(mmap+HNSW) + 교차참조 + 누적 업데이트 + 2-Pass 피드백 + 토픽 분할 + KG ego graph.

## 모달 진입점 매핑 (CLI / Tauri / MCP)

세 진입점이 동일 core 서비스를 공유하지만 노출 명령 셋과 책임이 다름.
...
| **shared/cli (Tauri 진입)** | `crates/shared/src/cli.rs:30` enum Commands | 12개 | Tauri app(`pipeline.exe`) 비-GUI 명령 위임 (init/start/stats/serve/batch 등) |
| **Tauri commands** | `modals/app/src/commands.rs` `#[tauri::command]` | 50개 | Dashboard 7탭 + 시스템 트레이 백엔드 |
| **MCP 도구** | `crates/shared/src/mcp_server.rs` make_tool | 11개 | Claude Code stdio 통합 (search/get_document/kg_*/등) |

**진입점 경계**:
- modals/cli는 **별도 바이너리** (자동화·CI·벤치마크 전용)
- modals/app은 **Tauri GUI** + 비-GUI 명령은 shared/cli로 위임
- MCP는 `pipeline serve` 또는 modals/cli `Serve`로 stdio 진입
...
- ✅ 단계 2: module-notify 분리 — raw send_text만, ProcessingSummary→text 포매팅은 file-pipeline `notification/format.rs` 분리
...
| 앱 | 단일 바이너리: CLI(pipeline) + GUI(Tauri). MCP는 `serve` 커맨드로 통합 |
...
| MCP | **25개 도구** (Phase 102 optimize 신규 — 비전문가용 통합 진입점 + 24 기존). 다차원 분류(Phase 92 H3). **현행 수치는 `prd/roadmap.md`** |
...
| 검색 테스트 | 13 시나리오 (랭킹/필터/하이브리드/MCP/MRR/엣지케이스/P@k/교차참조품질) |
...
| MCP 로깅 | `[mcp-usage]` search/get_document 호출 로깅 (실사용 측정 지표) |
...
MCP stdio (serve) →   McpState (11 tools)         →   StoragePort (zstd)
...
```
start / init / show-config / stats / doctor / memo / export / todo / kg / backfill-vec / topic-revise / serve
```
...
- MCP 서버는 CLI `pipeline serve`로 통합 (modals/mcp/ 삭제됨)

공유 로직은 crates/shared/ (config, build_service, cli, mcp_server).
REST 서버 완전 제거 (rest_server.rs 삭제) → APP은 50개 Tauri commands로 직접 Rust 호출.
UI 단일 진입점: `ui/index.html` (dashboard.html 삭제됨).
단일 바이너리: APP(Tauri)에 CLI+MCP가 인자 분기로 통합 (Tauri 19.4MB / CLI 15.5MB, 2026-04-30 release 실측).
...
└── src/           — 모든 구현 산출물
    ...
    │   └── shared/    cli, config, credential_store, mcp_server, platform, settings_db, tray
    ├── modals/    — 사용자 인터페이스 (모달별 독립)
    │   ├── cli/       CLI 바이너리 + daemon + 테스트
    │   └── app/       Tauri GUI (Dashboard + 트레이) — MCP는 cli `serve` 통합
...
crates/adapters/ driven/(llm/claude+anthropic+openai+ollama+gemini+fallback+chunked_agent+prompts,
                         ...
                         notification/telegram+slack+composite,
                         verification/claude, preprocessing/composite+tests,
                         reranking/claude+null)
...
crates/shared/   cli.rs, config.rs(19섹션+FieldMeta+validate+search 그룹), credential_store.rs,
                 mcp_server.rs(32 tools), platform.rs, settings_db.rs (단일 SCHEMA + llm_cache + decision_log + c1_rule_thresholds + pii_patterns_user),
...
- 노출: MCP tool `auto_suggest_from_counters` + Tauri command 동명
- 임계값:
...
- 노출: MCP tool `accept_suggested_decision` / `reject_suggested_decision` + Tauri command 동명
- 동작:
...
- 노출: MCP tool `clear_llm_cache` + Tauri command 동명
